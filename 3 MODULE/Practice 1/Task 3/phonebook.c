...
#define UPDATE_FIELD(fld) \
    ...
    if (is_non_empty(buf)) { \
        strncpy(c->fld, buf, sizeof(c->fld)-1); \
        c->fld[sizeof(c->fld)-1] = '\0'; \
    }
...
static int pb_grow_to(PhoneBook *pb, int min_cap) {
    if (min_cap <= pb->capacity)
        return 0;
    int nc = pb->capacity;
    while (nc < min_cap)
        nc *= 2;
    Contact *t = realloc(pb->contacts, (size_t)nc * sizeof(Contact));
    if (!t)
        return -1;
    pb->contacts = t;
    pb->capacity = nc;
...
void pb_load(PhoneBook *pb, const char *path) {
    int fd = open(path, O_RDONLY); //открываем файл только для чтения
    if (fd < 0) {
        if (errno != ENOENT)
            perror(path);
        return;
    ...
    Contact c;
    ssize_t r; 
    // читаем по одной записи из файла
    while ((r = read(fd, &c, sizeof c)) == (ssize_t)sizeof c) {
        if (pb_grow_to(pb, pb->count + 1) != 0) {
            fputs("Не хватило памяти при загрузке файла.\n", stderr);
            break;
        }
    ...
    // если r > 0 - в конце файла осталась неполная запись, которую пропускаем
    if (r > 0 && r != (ssize_t)sizeof c)
        fprintf(stderr, "%s: неполная запись в конце файла пропущена.\n", path);
    close(fd);
}

void pb_save(const PhoneBook *pb, const char *path) {
    int fd = open(path, O_WRONLY | O_CREAT | O_TRUNC, 0644); //открываем файл для записи если его нет - создаем если есть - очищаем
    if (fd < 0) {
        perror(path);
        return;
    }
    for (int i = 0; i < pb->count; i++) {
        ...
        ssize_t w = write(fd, &pb->contacts[i], sizeof(Contact));
        if (w != (ssize_t)sizeof(Contact)) {
            perror("write");
            break;
        }
    }
